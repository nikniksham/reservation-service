...
Также написал тест, который проверяет работоспособность моего retry - 30 синхронизированных потоков одновременно стартуют попытку забронировать 1 единицу товара (всего товара 10 единиц на складе). Все срабатывает, однако, если в ручке сделать прямой вызов брони товара (без retry обертки) - тест падает 
